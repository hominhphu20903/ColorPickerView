...
abstract class ColorSlider(context: Context, attributeSet: AttributeSet?) :
    ...
    protected val pickerOutlinePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        style = Paint.Style.STROKE
    }
    ...
    var thumbStrokeSize = dp(2)
        set(value) {
            field = value
            invalidate()
        }

    var thumbStrokeColor = Color.WHITE
        set(value) {
            field = value
            invalidate()
        }
    ...
    var thumbOutlineColor = Color.WHITE
        set(value) {
            field = value
            invalidate()
        }
        ...
        context.theme.obtainStyledAttributes(attributeSet, R.styleable.ColorPickerView, 0, 0)
            .apply {
                try {
                    ...
                    pickerBorderRadius = getDimension(
                        R.styleable.ColorPickerView_cpv_pickerBorderRadius,
                        thumbStrokeSize
                    )
                    pickerOutlineSize = getDimension(
                        R.styleable.ColorPickerView_cpv_pickerOutlineSize,
                        pickerOutlineSize
                    )
                    pickerOutlineColor = getColor(
                        R.styleable.ColorPickerView_cpv_pickerOutlineColor,
                        pickerOutlineColor
                    )

                    thumbOutlineSize = getDimension(
                        R.styleable.ColorPickerView_cpv_thumbOutlineSize,
                        thumbOutlineSize
                    )
                    thumbOutlineColor = getColor(
                        R.styleable.ColorPickerView_cpv_thumbOutlineColor,
                        thumbOutlineColor
                    )

                    thumbStrokeSize = getDimension(
                        R.styleable.ColorPickerView_cpv_thumbStrokeSize,
                        thumbStrokeSize
                    )
                    thumbStrokeColor =
                        getColor(R.styleable.ColorPickerView_cpv_thumbStrokeColor, thumbStrokeColor)
                } finally {
                    recycle()
                }
            }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.drawCircle(
            circleX,
            drawingTop,
            heightHalf - thumbStrokeSize - thumbOutlineSize,
            circlePaint.apply {
                color = ColorUtils.blendARGB(circleColor, Color.BLACK, 0.1f)
            })

        canvas.drawCircle(
            circleX,
            drawingTop,
            heightHalf - thumbStrokeSize - thumbOutlineSize - dp(1),
            circlePaint.apply {
                color = circleColor
            })
